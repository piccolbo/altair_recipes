...
def heatmap(data, x=0, y=1, color=2, aggregate="average", height=300, width=400):
    """Generate a heatmap."""
    ...
    return (
        ...
        .encode(
            ...
            color=alt.Color(
                aggregate + "(" + color + "):Q", scale=alt.Scale(scheme="greenblue")
            ),
        )
    )
...
def count_heatmap(data, x=0, y=1, height=300, width=400):
    ...
    return heatmap(
        data, x=x, y=y, color="x", aggregate="count", height=height, width=width
    )
